...
extern int _opnum;
extern std::map<RegValue, u32> constprop_values;
void ReplaceByMov32(shil_opcode& op, u32 v);

void InsertMov32Op(DecodedBlock* block, const shil_param& rd, const shil_param& rs)
{
	shil_opcode op2(block->oplist[_opnum]);
	op2.op = shop_mov32;
	op2.rd = rd;
	op2.rd2 = shil_param();
	op2.rs1 = rs;
	...
	block->oplist.insert(block->oplist.begin() + _opnum + 1, op2);
	_opnum++;
}
...
bool ExecuteConstOp(DecodedBlock* block, shil_opcode* op)
{
			...
			&& op->rd.is_reg())
	{
		...
		u32 rs1 = op->rs1.is_imm() ? op->rs1._imm : 0;
		u32 rs2 = op->rs2.is_imm() ? op->rs2._imm : 0;
		u32 rs3 = op->rs3.is_imm() ? op->rs3._imm : 0;
		u32 rd;
		u32 rd2;
		...
		case shop_rocr:
			{
				...
				InsertMov32Op(block, op2_rd, shil_param(FMT_IMM, rd2));

				// the previous insert might have invalidated our reference
				op = &block->oplist[_opnum - 1];
				op->rd2 = shil_param();
			}
			break;
		case shop_shl:
		...
		case shop_div32s:
			{
				...
				InsertMov32Op(block, op2_rd, shil_param(FMT_IMM, res >> 32));
		...
		case shop_jdyn:
			{
				...
				block->oplist.erase(block->oplist.begin() + _opnum);
				_opnum--;
		...
		case shop_fsca:
			{
				...
				shil_param op2_rd =  shil_param((Sh4RegType)(op->rd._reg + 1));
				op2_rd.version[0] = op->rd.version[1];
				InsertMov32Op(block, op2_rd, shil_param(FMT_IMM, rd_1));
		...
		default:
			printf("unhandled constant op %d", op->op);
